  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <form onSubmit={submit} className="flex flex-col gap-3">
        <input className="border p-3 rounded" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
        ...
        <button className="rounded-full bg-primary text-white py-2" disabled={loading}>{loading ? 'Loading...' : 'Create account'}</button>
      </form>
    </div>
  );
}
